...
void ftgl::VertexBuffer::replaceVertices(size_t index, const char* pvertices, size_t vcount)
{
	assert(index < items.size());
	auto& item = items[index];
	assert (item.vcount == vcount);

	memcpy(&(vertices[item.vstart]), pvertices, vcount * vertex_stride);
	state = State::DIRTY;
}
